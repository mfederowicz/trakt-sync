...
func TestMoviesServiceGetMovieWatchNow(t *testing.T) {
	setup := Setup()
	defer setup.Teardown()

	setup.Mux.HandleFunc("/movies/tron-legacy-2010/watchnow/us", func(w http.ResponseWriter, r *http.Request) {
	...
	test.AssertNoDiff(t, map[string]*str.WatchNowSources{"us": {
		...
		StreamingRanks: &str.WatchNowRank{Rank: test.Ptr(5787), Delta: test.Ptr(-586), Link: str.String("https://www.justwatch.com/us/movie/tron-2-0")},
	}}, got)
}

func TestMoviesServiceGetMovieJustwatchLinks(t *testing.T) {
	setup := Setup()
	defer setup.Teardown()

	setup.Mux.HandleFunc("/movies/tron-legacy-2010/watchnow/justwatch_links/pl", func(w http.ResponseWriter, r *http.Request) {
		test.AssertMethod(t, r, http.MethodGet)
...
func TestMoviesServiceGetMovieWatchNowForbidden(t *testing.T) {
	setup := Setup()
	defer setup.Teardown()

	setup.Mux.HandleFunc("/movies/tron-legacy-2010/watchnow/justwatch_links/us", func(w http.ResponseWriter, _ *http.Request) {
