...
func TestYounifyHandlers(t *testing.T) {
	tests := []struct {
		...
		{name: "connections not open to API apps", handler: YounifyConnectionsHandler{}, options: str.Options{Action: consts.Connections},
			method: http.MethodGet, path: "/younify/connections", status: http.StatusUnauthorized, body: `{}`, wantErr: "younify is not open to API apps"},
		{name: "connect not open to API apps", handler: YounifyConnectHandler{}, options: str.Options{Action: consts.Connect, ServiceID: "netflix"},
			method: http.MethodPost, path: "/younify/connect", status: http.StatusUnauthorized, body: `{}`, wantErr: "younify connect: Trakt answered 401"},
